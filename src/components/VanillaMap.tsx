    ...
    useEffect(() => {
        const initMap = async () => {
            ...
            try {
                ...
                var fallbackCoords: Record<string, { lat: number, lng: number }> = {
                    ...
                    .then(data => {
                        if (data && data.length > 0) {
                            ...
                            if (geometry && (geometry.type === 'Polygon' || geometry.type === 'MultiPolygon')) {
                                ...
                                function processCoordinates(coordsArray: any[]) {
                                    for (var i = 0; i < coordsArray.length; i++) {
                                        if (Array.isArray(coordsArray[i][0])) {
                                            processCoordinates(coordsArray[i]); // Recurse for MultiPolygon/Holes
                                        } else {
                                            bounds.extend(new google.maps.LatLng(coordsArray[i][1], coordsArray[i][0]));
                                        }
                                    }
                                }
                                ...
                                var fbBounds = new google.maps.LatLngBounds();
                                fbBounds.extend(new google.maps.LatLng(parseFloat(bbox[0]), parseFloat(bbox[2]))); // SW: min_lat, min_lon
                                fbBounds.extend(new google.maps.LatLng(parseFloat(bbox[1]), parseFloat(bbox[3]))); // NE: max_lat, max_lon
        ...
        // Suppress `google` undefined TypeScript warnings locally since loader.importLibrary handles it
        initMap();
    }, [city, targetKey]);
