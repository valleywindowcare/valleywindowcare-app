...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const cityName = content ? content.city : formatTitle(city);
    ...
    const seoTitle = isCommercial
        ? `Commercial ${formattedName} in ${cityName}`
        : `${formattedName} in ${cityName}`;

    const seoDescription = isCommercial
        ? `Valley Window Care and Exterior Cleaning provides premium ${formattedName.toLowerCase()} and property maintenance for businesses in ${cityName} and surrounding areas.`
        : `Valley Window Care and Exterior Cleaning offers premium ${formattedName.toLowerCase()} for residential properties in ${cityName} and surrounding areas.`;
    ...
    return {
        ...
        openGraph: {
            title: seoTitle,
            description: seoDescription,
            url: `https://valleywindowcare.com/service-areas/${city}/${service}`,
            ...
            images: [
                {
                    ...
                    width: 1200,
                    height: 630,
                    alt: `${formattedName} Portfolio Showcase in ${cityName}`,
                }
            ],
            type: 'website',
        }
...
export default async function CityServicePage({ params }: PageProps) {
    ...
    const areaContent = serviceData.find((d) => d.type === 'service' && d.citySlug === city && d.serviceSlug === service);
    const cityName = areaContent ? areaContent.city : formatTitle(city);
    ...
    return (
        <>
            <section className="relative w-full min-h-[500px] flex flex-col items-center justify-center py-20 bg-slate-900 overflow-hidden">
                <img src={heroImageToUse || "/images/portfolio/house-washing.webp"} alt={`${formattedName} Overview`} className="absolute inset-0 w-full h-full object-cover z-0 opacity-50" />
                ...
                <script
                    ...
                    dangerouslySetInnerHTML={{
                        __html: JSON.stringify({
                            ...
                            "description": `Valley Window Care and Exterior Cleaning provides premium ${formattedName.toLowerCase()} and property maintenance for homes and businesses in ${cityName}.`,
                        ...
                        __html: JSON.stringify({
                            ...
                            "itemListElement": [
                                ...
                                },
                                {
                                    ...
                                    "position": 2,
                                    "name": "Service Areas",
                                    "item": "https://valleywindowcare.com/service-areas"
                                },
                                {
                                    "@type": "ListItem",
                                    "position": 3,
                                    "name": cityName,
                                    "item": `https://valleywindowcare.com/service-areas/${city}`
                                },
                                {
                                    "@type": "ListItem",
                                    "position": 4,
                                    "name": formattedName,
                                    "item": `https://valleywindowcare.com/service-areas/${city}/${service}`
                                }
                            ]
                        })
                    }}
