...
const services = [
    { title: "Residential Rust Removal", slug: "rust-removal", image: "/site-gallery/authentic-IMG_5941.jpg" },
    { title: "Soft Wash", slug: "soft-washing", image: "/site-gallery/authentic-IMG_8432.jpg" },
    { title: "Driveway Cleaning", slug: "driveway-cleaning", image: "/gallery/audit-replacements/audit-1772396199144-839.webp" },
    { title: "Solar Panel Cleaning", slug: "solar-panel-cleaning", image: "/gallery/audit-replacements/audit-1772396199746-444.webp" },
    { title: "Commercial Awning Cleaning", slug: "professional-awning-cleaning", image: "/gallery/audit-replacements/audit-1772396199407-371.webp" }
];

const APP_DIR = path.join(process.cwd(), 'src/app', 'services');

services.forEach(svc => {
    const targetFolder = path.join(APP_DIR, svc.slug);
...
export const metadata: Metadata = {
    title: "${svc.title} | Valley Window Care and Exterior Cleaning",
    description: "Valley Window Care and Exterior Cleaning provides premium ${svc.title} in Northeast Wisconsin.",
    alternates: {
        canonical: "https://valleywindowcare.com/services/${svc.slug}"
    }
};

export default function ${svc.slug.replace(/-/g, '')}Page() {
    return (
        <>
            ...
            <Process isCommercial={${svc.slug.includes('commercial') ? 'true' : 'false'}} />
            <ReviewSlider />
        </>
    );
}
`;
    ...
    fs.writeFileSync(path.join(targetFolder, 'page.tsx'), pageContent, 'utf8');
    console.log(`✅ Generated Service Page: /services/${svc.slug}`);
});
