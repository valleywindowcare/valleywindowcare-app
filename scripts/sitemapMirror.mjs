...
async function scrapeBlogs() {
    ...
    try {
        ...
        try {
            await sitemapPage.goto('https://valleywindowcare.com/post-sitemap.xml', { waitUntil: 'domcontentloaded', timeout: 45000 });
        } catch (err) {
            console.log("[WAF XML REDIRECT] Caught XML redirect.");
            await new Promise(r => setTimeout(r, 8000));
        }

        let retries = 0;
        let sitemapContent = await sitemapPage.content();
        while ((sitemapContent.includes('sgcaptcha') || sitemapContent.includes('Robot Challenge')) && retries < 5) {
            console.log(`[WAF WARN] XML Sitemap blocked by Captcha. Delaying 10s and reloading... (Attempt ${retries + 1})`);
            await new Promise(r => setTimeout(r, 10000));
            await sitemapPage.reload({ waitUntil: 'networkidle0' });
            sitemapContent = await sitemapPage.content();
            retries++;
        }
    ...
    // HARD FAILSAFE FOR THE 20-POST GUARANTEE: If the sitemap XML refuses to parse due to WAF, we merge with the known baseline list.
    if (postUrls.length < 20) {
        console.log(`[WAF FAILSAFE] Merging active endpoints from baseline to guarantee the rigid >20 quota...`);
        try {
    ...
    for (const postUrl of postUrls) {
        try {
            ...
            try {
                ...
                if (err.message.includes('Execution context was destroyed') || err.message.includes('Navigation timeout')) {
                    console.log(`[WAF REDIRECT] Caught WAF redirect for ${postUrl}. Waiting for navigation to settle...`);
                    await new Promise(r => setTimeout(r, 8000));
                } else {
                    throw err;
                }
            ...
            // Bypass aggressive WAF checks
            let retries = 0;
            let currentHtml = await page.content();
            while ((currentHtml.includes('sgcaptcha') || currentHtml.includes('Robot Challenge') || currentHtml.includes('Just a moment...')) && retries < 3) {
                console.log(`[WAF WARN] Captcha blocked. Delaying 10s and reloading... (Attempt ${retries + 1})`);
                await new Promise(r => setTimeout(r, 10000));
            ...
            if (currentHtml.includes('sgcaptcha') || currentHtml.includes('Robot Challenge')) {
                console.log(`[WAF FATAL] Captcha impenetrable for ${postUrl}. Dropping to avoid duplication.`);
                continue;
            }

            await new Promise(r => setTimeout(r, 3000)); // allow Elementor lazy loaders
