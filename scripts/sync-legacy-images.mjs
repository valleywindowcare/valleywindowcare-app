...
async function run() {
    console.log("Starting Legacy Asset Re-Sync...");
    ...
    const tempDir = path.join(cwd, 'tmp-images');
    ...
    for (let i = 0; i < blogs.length; i++) {
        ...
        // 1. Try to fetch from legacy site
        const urlsToTry = [
            `https://valleywindowcare.com/${blog.slug}/`,
            `https://valleywindowcare.com/blog/${blog.slug}/`
        ];
        ...
        for (const url of urlsToTry) {
            try {
                ...
                if (res.ok) {
                    ...
                    if (ogImage && !ogImage.includes('upscalemedia-transformed')) { // exclude default placeholders
                        console.log(`Found legacy image: ${ogImage}`);
                        await downloadImage(ogImage, tempOriginalPath);
                        acquiredImagePath = tempOriginalPath;
                        sourceUsed = 'Legacy Site';
                        break; // Success
                    }
                }
            } catch (err) {
        ...
        // 3. Last Resort Fallback to Logo (Find a logo from public/ or media folders)
        if (!acquiredImagePath) {
