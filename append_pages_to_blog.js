...
let dumpFilePath = path.join(currentDir, 'wp_pages_dump.json');

// Check desktop if not in current
if (!fs.existsSync(dumpFilePath)) {
  dumpFilePath = path.join(process.env.HOME || '/Users/james', 'Desktop', 'wp_pages_dump.json');
}

if (!fs.existsSync(dumpFilePath)) {
  console.error(`❌ Could not find wp_pages_dump.json in ${currentDir} or Desktop. Please verify file location.`);
  process.exit(1);
}
...
if (numAdded === 0) {
  ...
  // Find the exact place to inject. Let's look for the end of the array `];`
  // Actually, let's inject right before the `];` that closes `export const blogData: BlogPost[] = [`
